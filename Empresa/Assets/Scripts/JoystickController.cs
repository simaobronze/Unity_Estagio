...
public class JoystickController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else { droneCommand = k_stopCommand; }
        if (!droneCommand.Equals(_lastCommandSent)) 
        { 
            _lastCommandSent = droneCommand;
            if (_streamScriptableObject.ActiveUser != null) 
            {
                User user = _streamScriptableObject.ActiveUser;
                if (user.streams.Count != 0) 
                {
                    //Debug.Log("MQTT DEBUG -> " + user.streams[0].device_id + " " + user.streams[0].driver_device_id);
                    //_mqttClient.PublishDroneCommand(droneCommand, user.streams[0].device_id, user.streams[0].driver_device_id);
                }
            }
        }
        ToggleGrabbable();
